...
def test_deal_document_read_resolves_legacy_file_url_unchanged():
    ...
    doc = DealDocumentRead.model_validate(data)
    assert doc.file_url == "/media/deal_documents/5_abc123.pdf"


@patch("app.services.s3_storage_service.boto3.client")
def test_deal_document_read_resolves_s3_key_to_presigned_url(mock_boto_client):
    from app.schemas.deal_document import DealDocumentRead
